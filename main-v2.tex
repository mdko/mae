...
\begin{frame}{Dependent Types}
    ...
    \begin{itemize}
        ...
        \item \todo{Explain inherent undecidability}
    \end{itemize}
\end{frame}
...
Spatial Safety
\begin{itemize}
    \item Unsafe languages like C conflate arrays and pointers, casts eliminate any semblance of type guarantees \note{types are just for size}
    \item Completely detecting buffer overflow in general is undecidable \note{so we try runtime things}
    \item Fat pointers: complete spatial safety at cost of compatibility
    \item Referent objects: partial safety and better compatibility
    \item Dependent types: prove in-bounds access at compile-time, emit run-time checks for full safety
    \item Solution seems to be (besides type-safe language):
        \begin{itemize}
            \item Restrict C
