...
Potential pointer dereference problems:
  \begin{itemize}
      \item Null
      \item Uninitialized
      \item Out-of-bounds \note{related: data might not be an array}
...
\begin{itemize}
    ...
    \item do not handle arbitrary casts % TODO why not
    \item Library wrapper calls for checked pointers % TODO is this any different from fat? \note{modified by external libraries must have metadata updated at the library call via wrappers}
    \item 1.63x overhead
\end{itemize}
\vspace{0.1in}
...
% TODO add info down here, up there...
...
% TODO note: MSCC: \item Unique capability attached to each memory block % for temporal errors, all I'll say right now
...
%        \item metadata tracked per object, associated with location of object in memory
        \item Advantages:
        \begin{itemize}
...
%TODO not talking about temporal yet: mention in 2nd half \item Incomplete temporal safety: dangling pointer within bounds of newly allocated object

%    - very good example: (top of pg 4)
%      """
...
\note[itemize]{
\item "Because nested objects (e.g., an array of structures) are assigned base and bound addresses that span the entire allocated region, it is impossible to detect sub-object overflows if an out-of-bounds pointer to an inner object remains within bounds of the outer object."
}
...
% Finish with hybrid approach (Softbound and Checked C?)
