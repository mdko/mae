...
\begin{itemize}[<+->]
    ...
    \item C: the de-facto standard
        \begin{itemize}
            ...
            \item Complete mutability via pointers % TODO define what I mean here
            \item Performant
        \end{itemize}
...
SafeC \footcite{austin_efficient_1994}:
  \begin{itemize}
     ...
     \item Fields include:
          \begin{itemize}
             \item base, size, storage class (\todo{add classes}) %errant deallocations)
             \item unique capability issued to an allocation % TODO why
          \end{itemize}
     \item Insert array access checks before each pointer/array dereference
     \item Complete spatial safety as long as
         \begin{itemize}
            \item transparent storage management % TODO revisit what I mean by this
...
Cyclone \footcite{jim_cyclone:_2002}: % offshoot of TAL/popcorn
      \begin{itemize}
          \item Array vs non-array pointers (never-null, fat, unitialized warning via control-flow analysis) % TODO lookup types (what does fat correspond to)
...
Avoid fat pointers by \alert{separating metadata} from pointer % metadata = information aboutn pointer, the base and bounds
\\
MSCC \footcite{xu_efficient_2004}
    \begin{itemize}
        \item Shadow structures mirror entire original structure % TODO what do I mean here? can I say it better
...
%        \item \todo{fix disjoint shadow data structure by ...}
        \item Handles arbitrary casts, sub-object overflows not covered by MSCC % TODO what do I mean here?
...
\begin{itemize}
    \item Leverage LCLint \todo{?}, an annotation-assisted buffer detection tool
...
\end{itemize}
    ...
    \vspace{0.2in}
\end{frame}
% TODO read up on ETC, cssv again
...
% TODO gives decent examples of use of LCLint
...
\begin{frame}{Linear Types}
    ...
    \begin{itemize}
        ...
        \item In imperative setting, non-duplicatable resource is the \emph{world} % don't want to world states at same time
...
\begin{frame}{Some Examples}
  LCL \footcite{evans_static_1996}
    \begin{itemize}
        ...
        \item $\texttt{dependent}$ annotation for sharing; programmer must ensure lifetimes are contained properly % lifetime of dependent is within lifetime of owner
    \end{itemize}
...
\begin{frame}{Rust}
    ...
    % TODO fix commas between footnote superscripts
...
% performing certain actions on a resource (reading and writing a memory location) should require a 'right' 
% or capability that is uniquely owned by one alias at any point during the execution of the program.
% althouh right is uniquly owned, it cn be transferred from one alias to another
% can be shared in a controlled manner
