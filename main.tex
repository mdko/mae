...
\begin{itemize}[<+->]
    ...
    \item C: the de-facto standard
        \begin{itemize}
            \item Data structure representation control % how it looks
            \item Memory management control % where it is
            \item Complete mutability via pointers % with systems without protected address space, can literally write anything; embedded have memory-mapped I/O
            \item Performant
            \item Legacy % alas: have to deal with it
        \end{itemize}
...
\footnotesize{
    \begin{itemize}
        ...
        \item Library wrapper calls for checked pointers % modified by external libraries must have metadata updated at the library call via wrappers
        \item Unique capability attached to each memory block % for temporal errors, all I'll say right now
        \item 1.63x overhead
    \end{itemize}
}
\begin{lstlisting}[language=C,mathescape,basicstyle={\scriptsize\ttfamily}]
...
% TODO gives decent examples of use of LCLint of CSSV
\begin{frame}[fragile]{CSSV Example}
  \footnotesize
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
        Foo bar baz
